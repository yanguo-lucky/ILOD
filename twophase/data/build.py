...
def divide_label_unlabel(
    ...
    # Add 'is_source' to labeled data
    for i in range(len(dataset_dicts)):
        if i in labeled_idx:
            # If this is a labeled sample, add 'is_source' = True
            sample_dict = dataset_dicts[i].copy()  # Ensure not to modify original dataset
            sample_dict["is_source"] = True  # Mark as source domain
            label_dicts.append(sample_dict)
        else:
...
# uesed by supervised-only baseline trainer 构建 仅有标签的 训练数据加载器
...
# uesed by unbiased teacher trainer  #两种剪裁
def build_detection_semisup_train_loader_two_crops(cfg, mapper=None):
    #应该是False代码会将整个训练数据集 (TRAIN) 划分为标签和无标签数据集。
