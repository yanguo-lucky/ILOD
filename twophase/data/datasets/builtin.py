...
def register_all_cityscapes_foggy(root):
    ...
    #_RAW_CITYSCAPES_SPLITS 是一个预定义的字典，其中包含了 Cityscapes Foggy 数据集的分组信息（如 train, val, test）及其对应的图像路径和标注路径
    for key, (image_dir, gt_dir) in _RAW_CITYSCAPES_SPLITS.items():
        #key 是数据集的名称（如 cityscapes_foggy_train），image_dir 是存储图像的目录，gt_dir 是存储标注的目录
