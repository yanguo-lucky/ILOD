...
from twophase.modeling.custom_losses import ConBoxLosses
from twophase.data.transforms.night_aug import NightAug
from twophase.data.transforms.light_aug import ImagePreprocessor
from twophase.modeling.da_heads.da_heads import DomainAdaptationModule_triplet
import copy
...
class TwoPCTrainer(DefaultTrainer):
    def __init__(self, cfg):
        ...
        self.conbox_losses=ConBoxLosses()
        self.light_aug = ImagePreprocessor()
        self.register_hooks(self.build_hooks())
        self.dagrl=DomainAdaptationModule_triplet(cfg)
    ...
    def resume_or_load(self, resume=True):
        ...
        if isinstance(self.model, DistributedDataParallel):
          # Check for PyTorch version >= 1.7
          for param in self.model.parameters():
                dist.broadcast(param.data, src=0)
            # Manually synchronize parameters
    ...
    def run_step_full_semisup(self):
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            record_dict, _, _, _ ,featuress= self.model(
        ...
        # Student-teacher stage
            ...
            record_all_label_data, _, _, _ ,featuress= self.model(
                label_data, branch="supervised"
            )
            ...
            # print(f"sf1.shape:{sf1.shape}")
            # print(f"sf2.shape:{sf2.shape}")
            # print(f"sf3.shape:{sf3.shape}")
            ...
            with torch.no_grad():
                ...
                unlabel_data_aug = self.light_aug.process_images_in_batch(unlabel_data)
                #unlabel_data_aug =unlabel_data
                (
                    _,
                    proposals_rpn_unsup,
                    proposals_roih_unsup,
                    _,
                 ) = self.model_teacher(unlabel_data_aug, branch="unsup_data_weak")
            ...
            # print(unlabel_data[0]['image'].shape)
            for item in label_data:
                item["is_source"] = True
            # print(len(label_data))
            targets = unlabel_data + label_data
           ...
           featuress)= self.model(
            ...
            img_features1 = [sf1, tf1]
            img_features2 = [sf2, tf2]
            img_features3 = [sf3, tf3]
            
            da_img_loss1 = self.dagrl.DA_Img_component(img_features1, targets)
            da_img_loss2 = self.dagrl.DA_Img_component(img_features2, targets)
            da_img_loss3 = self.dagrl.DA_Img_component(img_features3, targets)
            ...
            # print(f"roi_stu:{roi_stu}")
            # print(f"roi_teach:{roi_teach}")
            cons_loss = self.consistency_losses.losses(roi_stu,roi_teach)
            #conbox_loss = self.conbox_losses.losses(roi_stu, roi_teach)
            #print(conbox_loss)
            record_dict.update(cons_loss)
            #record_dict.update(conbox_loss)
            total_da_img_loss = {'da_img_loss': da_img_loss1['da_img_loss']*1.0 + da_img_loss2['da_img_loss']*0.5 + da_img_loss3['da_img_loss']*0.1}
            record_dict.update(total_da_img_loss)
#-------------------------------------------------------------------------------------------------------------
            # weight losses
            loss_dict = {}
            for key in record_dict.keys():
